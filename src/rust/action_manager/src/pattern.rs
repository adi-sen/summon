...
mod tests {
	...
	fn test_simple_pattern() {
		let pattern = "gh {query}";
		let input = "gh rust";

		let captures = match_pattern(pattern, input).unwrap();
		assert_eq!(captures.get("query"), Some(&"rust".to_string()));
	}

	#[test]
	fn test_complex_pattern() {
		let pattern = "gh {owner}/{repo}";
		let input = "gh rust-lang/rust";
	...
	fn test_no_match() {
		...
		assert!(match_pattern(pattern, input).is_none());
	}

	#[test]
	fn test_template_expansion() {
		let mut captures = HashMap::new();
		captures.insert("owner".to_string(), "rust-lang".to_string());
		captures.insert("repo".to_string(), "rust".to_string());

		let template = "https://github.com/{owner}/{repo}";
		let result = expand_template(template, &captures);
